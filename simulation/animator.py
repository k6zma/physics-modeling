...
def animate_trajectory(x_full, y_full, x_flight, y_flight, start_point, end_point, v0_min, total_frames=200, fps=24):
    ...
    ax.plot(x_flight, y_flight, label='Траектория после отрыва', linestyle='--', color='orange')
    ax.axhline(y=0, color='black', linestyle='-', linewidth=2)
    ax.set_xlabel('X, м')
    ax.set_ylabel('Y, м')
    ax.set_title('Анимация движения тела на дуге и движение после отрыва')

    ax.legend(["Движение по дуге", "Траектория после отрыва", f"Минимальная скорость: {v0_min:.2f} м/с"])

    ax.plot(start_point[0], start_point[1], 'o', color='#ABDF21', markersize=8, label="Начало дуги")
    ax.plot(end_point[0], end_point[1], 'o', color='#ABDF21', markersize=8, label="Конец дуги")
    ...
    with tempfile.NamedTemporaryFile(suffix=".gif", delete=False) as temp_file:
        ...
        with stqdm(total=total_frames) as pbar:
            ...
            ani.save(temp_file.name, writer=writer, progress_callback=lambda i, n: pbar.update(1))
        temp_file.seek(0)
